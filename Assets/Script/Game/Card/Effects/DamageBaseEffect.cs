...
namespace Game.Card.Effects
{
    /// <summary>
    /// 전역 베이스 공격 카드 효과
    /// - IBaseManager를 통해 대상 베이스를 찾아 HealthComponent.TakeDamage를 호출합니다.
    /// </summary>
    public class DamageBaseEffect : IVFXAwareEffect
    {
        private readonly DamageBaseEffectDefinition _definition;

        public EffectType EffectType => EffectType.DamageBase;
        public int Priority => _definition?.Priority ?? 0;

        public DamageBaseEffect(DamageBaseEffectDefinition definition)
        {
        ...
        public bool CanExecute(Vector2Int targetPos, GameContext context)
        {
            if (_definition == null)
            {
                Debug.LogWarning("[DamageBaseEffect] Invalid definition");
                return false;
            }

            if (context == null)
            {
                Debug.LogWarning("[DamageBaseEffect] Invalid GameContext");
                return false;
            }

            if (_definition.DamageAmount <= 0)
            {
                Debug.LogWarning("[DamageBaseEffect] DamageAmount must be greater than zero");
                return false;
            }
        ...
        public void ExecuteWithVFXData(Vector2Int targetPos, GameContext context, VFXTriggerData triggerData)
        {
            ...
            {
                Debug.Log($"[DamageBaseEffect] Attack failed: {triggerData.ValidationFailureReason}");
                return;
            }
        ...
        private void ExecuteInternal(GameContext context)
        {
            ...
            {
                Debug.LogWarning("[DamageBaseEffect] Cannot execute");
                return;
            }
            ...
            if (baseManager == null)
            {
                Debug.LogError("[DamageBaseEffect] IBaseManager not found in ServiceLocator");
                return;
            }
            ...
            {
                Debug.LogWarning("[DamageBaseEffect] Target team is None");
                return;
            }
            ...
            {
                Debug.LogWarning("[DamageBaseEffect] Target base or health component is null");
                return;
            }

            var health = targetBase.HealthComponent;
            health.TakeDamage(_definition.DamageAmount);

            Debug.Log($"[DamageBaseEffect] Dealt {_definition.DamageAmount} damage to {targetTeam} base");
        }
